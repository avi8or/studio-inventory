...
const appComponent = await readFile(new URL("../src/App.vue", import.meta.url), "utf8");
...
test("print labels use the requested information hierarchy", () => {
  assert.match(
    appComponent,
    /print-label-card[\s\S]*label-manufacturer[\s\S]*label-paper-line[\s\S]*label-form-size[\s\S]*BarcodeSvg[\s\S]*label\.label_code[\s\S]*label-sku/,
  );
});
